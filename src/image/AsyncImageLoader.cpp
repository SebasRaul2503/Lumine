...
void AsyncImageLoader::request(const QString& path)
{
    ...
    m_pool->start(QRunnable::create([this, path, generation]() {
        ...
        // Hop back to the owning thread; deliver() decides whether this
        // result is still wanted.
        QMetaObject::invokeMethod(
            this,
            [this, path, generation, result]() {
                deliver(path, generation, result.ok(), result.image, result.message);
            },
            Qt::QueuedConnection);
    }));
}
...
{
    ++m_generation; // any pending result is now stale
    m_pool->clear();
}
